...
export function getStore(): string | undefined {
  return process.env.SHOPIFY_STORE || loadProfile().store;
}

export function setStore(store: string): void {
...
export function getAccessToken(): string | undefined {
  return process.env.SHOPIFY_ACCESS_TOKEN || loadProfile().accessToken;
}
...
export function getApiVersion(): string {
  return process.env.SHOPIFY_API_VERSION || loadProfile().apiVersion || DEFAULT_API_VERSION;
}
...
/**
 * Normalize store name to proper format
...
export function normalizeStoreName(store: string): string {
  ...
  let normalized = store.replace(/^https?:\/\//, '');
