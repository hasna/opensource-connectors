...
// Config directory: ~/.connect/connect-googlecalendar/
...
export function getAccessToken(): string | undefined {
  return process.env.GOOGLE_CALENDAR_ACCESS_TOKEN || loadProfile().accessToken;
}
...
export function getRefreshToken(): string | undefined {
  return process.env.GOOGLE_CALENDAR_REFRESH_TOKEN || loadProfile().refreshToken;
}
...
export function setTokens(tokens: { accessToken: string; refreshToken?: string; expiresIn?: number }): void {
  ...
  if (tokens.expiresIn) {
    config.tokenExpiry = Date.now() + tokens.expiresIn * 1000;
  }
  saveProfile(config);
}
...
export function isTokenExpired(): boolean {
  ...
  return Date.now() >= config.tokenExpiry - 60000; // 1 minute buffer
}
