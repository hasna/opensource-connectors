...
import type { CliConfig, OAuth2Tokens } from '../types/index.ts';
...
// Store config in ~/.connect/connect-googledrive/ (always in home directory)
function resolveBaseConfigDir(): string {
  return join(homedir(), '.connect', 'connect-googledrive');
}
...
export function getClientId(): string | undefined {
  return process.env.GOOGLE_CLIENT_ID || loadBaseConfig().clientId;
}

export function getClientSecret(): string | undefined {
  return process.env.GOOGLE_CLIENT_SECRET || loadBaseConfig().clientSecret;
}
...
export function getAccessToken(): string | undefined {
  return process.env.GOOGLE_ACCESS_TOKEN || loadTokens()?.accessToken;
}

export function getRefreshToken(): string | undefined {
  return process.env.GOOGLE_REFRESH_TOKEN || loadTokens()?.refreshToken;
}
