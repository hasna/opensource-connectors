...
// Config directory: ~/.connect/midjourney/
...
export function getApiKey(): string | undefined {
  return process.env.MIDJOURNEY_API_KEY || loadProfile().apiKey;
}
...
export function getDiscordToken(): string | undefined {
  return process.env.DISCORD_TOKEN || loadProfile().discordToken;
}

export function setDiscordToken(discordToken: string): void {
  const config = loadProfile();
...
export function getBaseUrl(): string | undefined {
  return process.env.MIDJOURNEY_BASE_URL || loadProfile().baseUrl;
}
