import type { SentryConfig, OutputFormat } from '../types';
import { SentryApiError } from '../types';
...
export class SentryClient {
   ...
   * Sentry uses Bearer token authentication
   */
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new SentryApiError(errorMessage, response.status);
    }
