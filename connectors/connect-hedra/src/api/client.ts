import type { HedraConfig, OutputFormat } from '../types';
import { HedraApiError } from '../types';

// Hedra API base URL
const DEFAULT_BASE_URL = 'https://api.hedra.com';
...
export class HedraClient {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    // Hedra uses Bearer token authentication
    ...
    if (!response.ok) {
      ...
      throw new HedraApiError(errorMessage, response.status);
    }
