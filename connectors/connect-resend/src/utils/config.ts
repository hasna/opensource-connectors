...
export function getApiKey(): string | undefined {
  return process.env.RESEND_API_KEY || loadProfile().apiKey;
}
...
export function getProfileExportsDir(profile?: string): string {
  const profileName = profile || getCurrentProfile();
  return join(getProfileDir(profileName), 'exports');
}

export function getProfileImportsDir(profile?: string): string {
  const profileName = profile || getCurrentProfile();
