...
program
  .name(CONNECTOR_NAME)
  .description('HeyGen API connector CLI - AI video generation with avatars')
  .version(VERSION)
  ...
  .hook('preAction', (thisCommand) => {
    ...
    if (opts.apiKey) {
      process.env.HEYGEN_API_KEY = opts.apiKey;
    }
  });
...
function getClient(): HeyGen {
  const apiKey = getApiKey();
  if (!apiKey) {
    error(`No API key configured. Run "${CONNECTOR_NAME} config set-key <key>" or set HEYGEN_API_KEY environment variable.`);
    process.exit(1);
  }
  return new HeyGen({ apiKey });
}
