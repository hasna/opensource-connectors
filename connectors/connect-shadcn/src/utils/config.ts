...
export function getPackageManager(): 'npm' | 'pnpm' | 'yarn' | 'bun' | undefined {
  return loadProfile().packageManager;
}

export function setPackageManager(pm: 'npm' | 'pnpm' | 'yarn' | 'bun'): void {
  const config = loadProfile();
