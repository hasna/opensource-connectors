...
// Docker Hub connector configuration
...
export interface ProfileConfig {
  ...
  password?: string;
  accessToken?: string;
  // Add more config fields as needed for Docker Hub API
}

// Store for --profile flag override (set by CLI before commands run)
let profileOverride: string | undefined;

// Config directory: ~/.connect/docker/
...
export function getUsername(): string | undefined {
  return process.env.DOCKER_USERNAME || loadProfile().username;
}
...
export function getPassword(): string | undefined {
  return process.env.DOCKER_PASSWORD || loadProfile().password;
}
...
export function getAccessToken(): string | undefined {
  return process.env.DOCKER_ACCESS_TOKEN || loadProfile().accessToken;
}
...
// Legacy getApiKey for compatibility - returns access token
