...
export function getApiKey(): string | undefined {
  return process.env.USPTO_API_KEY || process.env.USPTO_TOKEN || loadProfile().apiKey;
}
...
export function getHeadless(): boolean {
  const envVal = process.env.USPTO_HEADLESS;
  if (envVal !== undefined) {
    return envVal !== 'false';
  }
  const config = loadProfile();
...
export function getBrowser(): ProfileConfig['browser'] | undefined {
  return (process.env.USPTO_BROWSER as ProfileConfig['browser']) || loadProfile().browser;
}

export function setBrowser(browser: ProfileConfig['browser']): void {
  const config = loadProfile();
...
export function getOutputDir(): string | undefined {
  return process.env.USPTO_OUTPUT_DIR || loadProfile().outputDir;
}

export function setOutputDir(outputDir: string): void {
  const config = loadProfile();
