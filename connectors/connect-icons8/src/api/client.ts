import type { Icons8Config, OutputFormat } from '../types';
import { Icons8ApiError } from '../types';

// Icons8 API base URL
const DEFAULT_BASE_URL = 'https://api.icons8.com';
...
export class Icons8Client {
  ...
  private buildUrl(path: string, params?: Record<string, string | number | boolean | undefined>): string {
    ...
    // Icons8 API typically uses token as a query parameter
  ...
  /**
   * Make an authenticated request to the Icons8 API
   * Icons8 uses token-based authentication via query parameter
   */
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new Icons8ApiError(errorMessage, response.status);
    }
