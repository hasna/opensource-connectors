...
// Config directory: ~/.connect/maropost/
...
export function getApiKey(): string | undefined {
  return process.env.MAROPOST_API_KEY || loadProfile().apiKey;
}
...
export function getAccountId(): number | undefined {
  const envAccountId = process.env.MAROPOST_ACCOUNT_ID;
  if (envAccountId) {
    const parsed = parseInt(envAccountId, 10);
